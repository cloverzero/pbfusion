...
use models::{
    CreateProjectRequest, DiffDetail, DiffFilter, DiffItem, DiffType, ElementType, ListParams,
    Project, ProjectStatus, SettleDiffRequest, Settlement,
};
...
#[tauri::command]
fn list_projects(params: Option<ListParams>) -> Result<Vec<Project>, String> {
...
#[tauri::command]
fn get_project(id: u32) -> Result<Project, String> {
    let projects = storage::load_projects().map_err(|e| e.to_string())?;
    projects
        .into_iter()
        .find(|p| p.id == id)
...
async fn run_diff_analysis(
    ...
    loop {
        match (&source_next, &target_next) {
            (None, None) => break,
            (Some(_), None) => {
                // Remaining source elements are "Removed"
            ...
            (None, Some(_)) => {
                // Remaining target elements are "Added"
            ...
            (Some(src_el), Some(tgt_el)) => {
                ...
                match (src_type, tgt_type) {
                    _ if src_type == tgt_type && src_id == tgt_id => {
                        // Same element — check if modified
                    ...
                    _ if element_order(src_el) < element_order(tgt_el) => {
                        // Source element not in target → Removed
                        diffs.push(DiffItem {
                            id: diff_id,
                            project_id,
                            element_type: src_type,
                    ...
                    _ => {
                        // Target element not in source → Added
                        diffs.push(DiffItem {
                            id: diff_id,
                            project_id,
    ...
    if let Some(mut p) = projects.into_iter().find(|p| p.id == project_id) {
        ...
        // Notify frontend of project update
...
fn element_key(el: &Element) -> (ElementType, i64) {
    match el {
...
/// Quick structural equality check for PBF elements
...
#[tauri::command]
fn list_diffs(project_id: u32, filter: Option<DiffFilter>) -> Result<Vec<DiffItem>, String> {
...
#[tauri::command]
fn get_diff_detail(project_id: u32, diff_id: u32) -> Result<DiffDetail, String> {
    let diffs = storage::load_diffs(project_id).map_err(|e| e.to_string())?;
    let diff = diffs
        .iter()
        .find(|d| d.id == diff_id)
    ...
    // Get the project for file paths
    let projects = storage::load_projects().map_err(|e| e.to_string())?;
    ...
    // Open indexed readers
    let mut source_reader = pbf_craft::readers::IndexedReader::from_path(&project.source_path)
        .map_err(|e| format!("Failed to open source PBF: {}", e))?;
    let mut target_reader = pbf_craft::readers::IndexedReader::from_path(&project.target_path)
    ...
    Ok(DiffDetail {
        ...
        related: Vec::new(), // placeholder: no dependency tracking yet
    })
}
